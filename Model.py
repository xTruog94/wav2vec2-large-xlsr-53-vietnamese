...
from utils import open_json,to_json,open_pkl,to_pkl
import datasets
import numpy as np
import torch
...
class ModelArguments:
    ...
    vocab_file: Optional[str] = field(
        ...
        metadata={"help": "Whether to vocabulary of model"},
    )
...
class DataTrainingArguments:
    ...
    train_path_from_dict: str = field(
        default=None, metadata={"help": "The name of the training dataset to use (via the datasets library)."}
    )
    validation_path_from_dict: str = field(
    ...
    use_cache: Optional[bool] = field(
        default=True,
        metadata={"help": "Using cache data"},
    )
    data_cache_folder: Optional[str] = field(
        default='./cache',
        metadata={"help": "Directory include cache"},
    )
    train_cache_filename: Optional[str] = field(
        default='train_full.pkl',
        metadata={"help": "filename of train data processed"},
    )
    val_cache_filename: Optional[str] = field(
        default='val_full.pkl',
        metadata={"help": "filename of test data processed"},
    )
...
class DataCollatorCTCWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # print(label_features,self.padding,self.max_length_labels,self.pad_to_multiple_of_labels)
...
def main():
    ...
    if data_args.use_cache and \
        os.path.exists(os.path.join(data_args.data_cache_folder, data_args.train_cache_filename)) and \
        os.path.exists(os.path.join(data_args.data_cache_folder, data_args.val_cache_filename)):
        print("Loading data from cache")
        train_dataset = open_pkl(os.path.join(data_args.data_cache_folder, data_args.train_cache_filename))
        val_dataset = open_pkl(os.path.join(data_args.data_cache_folder, data_args.val_cache_filename))
        
    else:
        tmp_train_data = open_json(data_args.train_path_from_dict)
        tmp_test_data = open_json(data_args.validation_path_from_dict)
        train_dataset = datasets.Dataset.from_dict(tmp_train_data)
        val_dataset = datasets.Dataset.from_dict(tmp_test_data)
        ...
        train_dataset = train_dataset.map(prepare_example, remove_columns=[data_args.speech_file_column,data_args.audio_file_column])
        val_dataset = val_dataset.map(prepare_example, remove_columns=[data_args.speech_file_column,data_args.audio_file_column])
        ...
        to_pkl(os.path.join(data_args.data_cache_folder, data_args.train_cache_filename),train_dataset)
        to_pkl(os.path.join(data_args.data_cache_folder, data_args.val_cache_filename),val_dataset)
